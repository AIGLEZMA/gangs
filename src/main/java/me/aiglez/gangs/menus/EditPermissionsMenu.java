...
public class EditPermissionsMenu extends Gui {
  ...
  private static final MenuScheme BUTTONS =
      new MenuScheme().mask("000000000").mask("001111100").mask("000010000");
  ...
  public void redraw() {
    ...
    BUTTONS
        ...
        .acceptIfSpace(
            ItemStackBuilder.of(Material.STAINED_GLASS_PANE)
                .data(7)
                .buildConsumer(e -> e.setCancelled(true)));
    ...
    for (final Permissible.Permission permission : Permissible.Permission.values()) {
      ...
      setItem(
          currentSlot,
          ItemStackBuilder.of(state ? Material.EMERALD_BLOCK : Material.REDSTONE_BLOCK)
              .name(
                  Placeholders.replaceIn(
                      CONFIG.getNode("items", path, "name").getString(""),
                      permission.getCoolName()))
              ...
              .buildConsumer(
                  e -> {
                    ...
                    if (!state) {
                      user.messagec(
                          "permission.granted", permission.getCoolName(), rank.getCoolName());
                    } else {
                      user.messagec(
                          "permission.denied", permission.getCoolName(), rank.getCoolName());
                    }
                    redraw();
                  }));
